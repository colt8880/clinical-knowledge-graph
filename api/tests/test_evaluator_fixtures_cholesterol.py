"""Parametrized fixture tests for ACC/AHA 2018 Cholesterol fixtures.
...
Verification targets (from docs/build/23-accaha-cholesterol-subgraph.md):
  1. All 4 fixtures produce expected trace events.
...
FIXTURES_DIR = Path(__file__).resolve().parent.parent.parent / "evals" / "fixtures" / "cholesterol"

# All cholesterol fixture directories
...
class TestCholesterolFixtureExpectedOutcome:
    ...
    async def test_expected_recommendations(self, client, case_name: str):
        ...
        # Filter to ACC/AHA recommendations only for this single-guideline gate
        accaha_recs = [r for r in recs if r.get("guideline_id") == "guideline:acc-aha-cholesterol-2018"]
        ...
        # ACC/AHA rec count must match expected
        expected_accaha_count = len([r for r in expected_recs if r.get("guideline_id") == "guideline:acc-aha-cholesterol-2018"])
        assert len(accaha_recs) == expected_accaha_count, (
            f"ACC/AHA recommendation count mismatch in {case_name}: "
            f"expected {expected_accaha_count}, got {len(accaha_recs)}\n"
            f"Actual ACC/AHA recs: {accaha_recs}"
        )
